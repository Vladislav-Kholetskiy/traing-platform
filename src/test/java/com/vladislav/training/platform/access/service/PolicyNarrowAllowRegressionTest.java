...
/**
 * Проверяет, что {@code PolicyNarrowAllow} не уходит от текущих договорённостей.
 * Сценарии сосредоточены на важных регрессионных рисках.
 */
@ExtendWith(MockitoExtension.class)
class PolicyNarrowAllowRegressionTest {

    private static final Long AUTHORIZED_ACTOR_ID = 101L;
    private static final Long UNAUTHORIZED_ACTOR_ID = 202L;
    private static final Instant FIXED_INSTANT = Instant.parse("2026-05-07T23:00:00Z");
    ...
    @Test
    void authorizedAdministrativeActorShouldGainAdministrativeReadAccessWithoutGenericShortcuts() {
        ...
        when(queryActorResolver.resolveActorUserId()).thenReturn(AUTHORIZED_ACTOR_ID);
        when(utcClock.now()).thenReturn(FIXED_INSTANT);
        when(userOrgFoundationStateReadService.findUserAccessPolicyFoundationState(AUTHORIZED_ACTOR_ID, FIXED_INSTANT))
            .thenReturn(new UserOrgFoundationStateReadService.UserAccessPolicyFoundationState(
                AUTHORIZED_ACTOR_ID,
                true,
                Set.of("ADMIN")
            ));
        setAuthentication(authenticatedToken(AUTHORIZED_ACTOR_ID, "ROLE_ADMIN"));

        AccessPolicyQueryContextResolver resolver = new AccessPolicyQueryContextResolver(queryActorResolver, utcClock);

        assertAllowed(policy.resolveReadScope(resolver.resolveNotificationAdministrationContext(AUTHORIZED_ACTOR_ID)));
        assertAllowed(policy.resolveReadScope(resolver.resolveNotificationAdministrationDetailContext(AUTHORIZED_ACTOR_ID, 701L)));
        assertAllowed(policy.resolveReadScope(resolver.resolveNotificationRuleAdministrationContext(AUTHORIZED_ACTOR_ID)));
        assertAllowed(policy.resolveReadScope(resolver.resolveNotificationRuleAdministrationDetailContext(AUTHORIZED_ACTOR_ID, 702L)));
        assertAllowed(policy.resolveReadScope(resolver.resolveImportJobAdministrationContext(AUTHORIZED_ACTOR_ID)));
        assertAllowed(policy.resolveReadScope(resolver.resolveImportJobAdministrationDetailContext(AUTHORIZED_ACTOR_ID, 703L)));
        assertAllowed(policy.resolveReadScope(resolver.resolveImportJobItemAdministrationContext(AUTHORIZED_ACTOR_ID)));
        assertAllowed(policy.resolveReadScope(resolver.resolveImportJobItemAdministrationDetailContext(AUTHORIZED_ACTOR_ID, 704L)));
        assertAllowed(policy.resolveReadScope(resolver.resolveAuditEventAdministrationContext(AUTHORIZED_ACTOR_ID)));
        assertAllowed(policy.resolveReadScope(resolver.resolveAuditEventAdministrationDetailContext(AUTHORIZED_ACTOR_ID, 705L)));

        verifyNoInteractions(accessFoundationStateReadService);
    }

    @Test
    void unauthorizedActorMustRemainDeniedForAdministrativeAdministrativeReads() {
        JpaAccessSpecificationPolicy policy = new JpaAccessSpecificationPolicy(
            userOrgFoundationStateReadService,
            accessFoundationStateReadService,
            interactiveActorResolver
        ...
        when(utcClock.now()).thenReturn(FIXED_INSTANT);
        when(userOrgFoundationStateReadService.findUserAccessPolicyFoundationState(UNAUTHORIZED_ACTOR_ID, FIXED_INSTANT))
            .thenReturn(new UserOrgFoundationStateReadService.UserAccessPolicyFoundationState(
    ...
    @Test
    void administrativeAdministrativeReadAllowanceMustNotDependOnGenericOrCommandShortcuts() throws Exception {
        ...
        assertThat(source)
            ...
            .doesNotContain("notification receipt")
            .doesNotContain("allowAllAdministrative")
            .doesNotContain("permitAllAdministrative");
    }

    private void assertAllowed(AccessReadScope scope) {
