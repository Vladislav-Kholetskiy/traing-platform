...
/**
 * Реализация сервиса {@code SelfExecutionAdmissionFoundationStateReadServiceImpl}.
 */
class SelfExecutionAdmissionFoundationStateReadServiceImpl implements SelfExecutionAdmissionFoundationStateReadService {

    private final TestRepository testRepository;
    private final TopicRepository topicRepository;
    private final CourseRepository courseRepository;

    SelfExecutionAdmissionFoundationStateReadServiceImpl(
    ...
    @Override
    public SelfExecutionAdmissionFoundationState findSelfExecutionAdmissionFoundationState(Long actorUserId, Long testId) {
        ...
        try {
            ...
            requireSelfExecutionEligible(test);
            return new SelfExecutionAdmissionFoundationState(test.id());
    ...
    private Topic requirePublishedTopic(Topic topic) {
        if (topic.status() != ContentStatus.PUBLISHED) {
            throw new NotFoundException("Self execution topic not found: " + topic.id());
        }
        return topic;
    }
    ...
    private void requirePublishedCourse(com.vladislav.training.platform.content.domain.Course course) {
        if (course.status() != ContentStatus.PUBLISHED) {
            throw new NotFoundException("Self execution course not found: " + course.id());
        }
    }

    private void requireSelfExecutionEligible(Test test) {
        if (test.status() != ContentStatus.PUBLISHED || test.isActiveFinalForTopic()) {
            throw new NotFoundException("Self execution test not found: " + test.id());
        }
    }
}
