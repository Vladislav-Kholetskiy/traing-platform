...
/**
 * Реализация сервиса {@code SelfCurrentAttemptReadFoundationStateReadServiceImpl}.
 */
class SelfCurrentAttemptReadFoundationStateReadServiceImpl implements SelfCurrentAttemptReadFoundationStateReadService {

    private final TestRepository testRepository;
    private final TopicRepository topicRepository;
    private final CourseRepository courseRepository;

    SelfCurrentAttemptReadFoundationStateReadServiceImpl(
    ...
    @Override
    public SelfCurrentAttemptReadFoundationState findSelfCurrentAttemptReadFoundationState(Long actorUserId, Long testId) {
        ...
        try {
            ...
            requireSelfCurrentAttemptReadable(test);
            return new SelfCurrentAttemptReadFoundationState(test.id());
    ...
    private Topic requirePublishedTopic(Topic topic) {
        if (topic.status() != ContentStatus.PUBLISHED) {
            throw new NotFoundException("Self current-attempt topic not found: " + topic.id());
        }
        return topic;
    }
    ...
    private void requirePublishedCourse(com.vladislav.training.platform.content.domain.Course course) {
        if (course.status() != ContentStatus.PUBLISHED) {
            throw new NotFoundException("Self current-attempt course not found: " + course.id());
        }
    }

    private void requireSelfCurrentAttemptReadable(Test test) {
        if (test.status() != ContentStatus.PUBLISHED || test.isActiveFinalForTopic()) {
            throw new NotFoundException("Self current-attempt test not found: " + test.id());
        }
    }
}
